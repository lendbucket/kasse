...
/**
 * POST /api/onboarding/agreements/send-test
 *
 * Same as /send but all emails go to the authenticated owner's email
 * instead of the staff members'. The PDF + sign token are still real
 * (owner sees the exact email staff will see). This lets the owner
 * preview the signing flow before triggering real sends.
 */
  ...
  try {
    ...
    if (session.user.role !== 'OWNER') {
      return NextResponse.json(
        { error: 'forbidden', message: 'only OWNER can send test agreements' },
        { status: 403 }
      );
    }
    ...
    console.error('POST /api/onboarding/agreements/send-test failed', err);
