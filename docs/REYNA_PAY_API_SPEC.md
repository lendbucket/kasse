...
**Base URL:** https://app.salontransact.com/api/v1 (will migrate to https://api.reynapay.com/v1 once domain is provisioned)
...
3. **Idempotency on every mutating operation.** Network retries are not optional in payments — they happen constantly. Every POST, PATCH, and DELETE MUST accept an Idempotency-Key header and SHALL produce the same response when called with the same key + same body. See IDEMPOTENCY below.
...
Every POST, PATCH, and DELETE request to Reyna Pay MUST include an `Idempotency-Key` header. Requests without this header receive `400 Bad Request` with error code `MISSING_IDEMPOTENCY_KEY`.

GET requests do not require an idempotency key. They are inherently safe to retry.

### Key format

- UUID v4 (recommended), or any string of 8 to 64 characters
...
- **Same key + same request body** within the TTL window: engine returns the cached response. No side effect (the original operation already executed).
- **Same key + different request body** within the TTL window: engine returns `409 Conflict` with error code `IDEMPOTENCY_KEY_REUSED`. This indicates a consumer bug — the consumer should generate a new key.
...
Idempotency:
- `MISSING_IDEMPOTENCY_KEY` — POST/PATCH/DELETE without Idempotency-Key header
...
2. Check that `t` is within the last 5 minutes — reject if older (replay attack protection)
