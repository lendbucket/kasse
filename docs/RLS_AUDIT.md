...
| `lib/booking/public-context.ts` | — | PUBLIC_ANONYMOUS (helper) | Anonymous slug→org/location resolver. `resolvePublicContextBySlug` resolves by bookingSlug when provided (isActive-filtered), falling back to the oldest active location. `listPublicLocationsBySlug` lists active locations for the picker. Both use prismaAdmin for the cross-tenant lookup (kasse_app role has rolbypassrls=FALSE; plain prisma returns zero rows without a tenant GUC). Only exposes org/location display info, never customer data. |
